    ...
    return (
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                >
                    ...
                    <div className="divide-y divide-border/60">
                        {[
                            ...
                            <motion.div
                                key={i}
                                whileHover={{ x: 10, backgroundColor: "rgba(198, 167, 94, 0.05)" }}
                                className="p-6 cursor-pointer group flex items-center gap-5 transition-colors"
                            >
                                <div className="size-14 rounded-2xl bg-secondary/30 flex items-center justify-center shrink-0 group-hover:bg-white border border-transparent group-hover:border-accent/20 transition-all group-hover:text-accent shadow-sm">
                                ...
                                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-colors ${act.status === 'Publié' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}>
                                    {act.status}
                                </span>
                            </motion.div>
                        ))}
                ...
                >
                    {/* Message Card */}
                    <div className="bg-white p-8 rounded-[2rem] border border-border shadow-2xl shadow-primary/5 relative overflow-hidden group">
                        ...
                        <div className="space-y-4 relative z-10">
                            {[1, 2].map((i) => (
                                <div key={i} className="bg-secondary/20 p-4 rounded-2xl border border-border/40 hover:bg-white hover:border-accent/40 hover:shadow-lg transition-all cursor-pointer">
                                    <p className="text-[10px] font-black text-accent uppercase tracking-widest mb-1">Mme Diop • 10:15</p>
                                    <p className="text-sm font-bold text-primary line-clamp-1">Nouvelle demande de sponsoring pour le gala...</p>
                                </div>
                            ))}
                        </div>
